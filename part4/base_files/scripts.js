...
function generateCalendar(year, month) {
  ...
  function clearSelection() {
    days.forEach(day => day.classList.remove("range-start", "range-end", "in-range"));
  }
  ...
  function updateSelection() {
    ...
    for (let i = from; i <= to; i++) {
      if (i === from) days[i].classList.add("range-start");
      else if (i === to) days[i].classList.add("range-end");
      else days[i].classList.add("in-range");
    }

    // save dates to hidden inputs
    selectedCheckIn = days[from].dataset.date;
    selectedCheckOut = days[to].dataset.date;
